...
import {
  fetchContents,
  fetchCourseById,
} from "../../Components/Store/CourseSlice";
import { RiFileVideoLine, RiListCheck3 } from "react-icons/ri";
import { FaRegFileAlt } from "react-icons/fa";
import {
  createPayment,
  validatePayment,
} from "../../Components/Store/PaymentSlice";
  ...
  async function payHandler(e) {
    try {
      const response = await dispatch(
        createPayment({ id: courseInfo._id, amount, currency })
      );
      let order;
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          const res = await dispatch(
            validatePayment({ id: courseInfo._id, body })
          );
          ...
          <>
            <div className="desc-heading">
              ...
              <div className="desc-footer">
                <p>
                  <IoTimerOutline /> Duration: {courseInfo?.duration} months
                </p>
                <p>
                  {" "}
                  <PiStudent /> Students enrolled:{" "}
                  {courseInfo?.enrolled?.length}{" "}
                </p>
              </div>
            </div>
                        ...
                        <div className="lecture-content">
                          <p>
                            {contents.type === "lecture" ? (
                              <RiFileVideoLine style={{ color: "red" }} />
                            ) : contents.type === "assignment" ? (
                              <FaRegFileAlt style={{ color: "green" }} />
                            ) : (
